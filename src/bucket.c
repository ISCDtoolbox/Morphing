...
pBucket newBucket_3d(pMesh mesh,int nmax) {
    ...
    for (k=1; k<=mesh->np; k++) {
        ...
        ii = LS_MAX(0,(int)(dd * ppt->c[0])-1);
        jj = LS_MAX(0,(int)(dd * ppt->c[1])-1);
        kk = LS_MAX(0,(int)(dd * ppt->c[2])-1);
...
int buckin_3d(pMesh mesh,pBucket bucket,double *c) {
  pPoint    pp1;
    ...
    do {
        imin = LS_MAX(0,ii-d);
        imax = LS_MIN(ii+d,siz-1);
        jmin = LS_MAX(0,jj-d);
        jmax = LS_MIN(jj+d,siz-1);
        kmin = LS_MAX(0,kk-d);
        kmax = LS_MIN(kk+d,siz-1);
        for (k=kmin; k<=kmax; k++)
            for (j=jmin; j<=jmax; j++)
                for (i=imin; i<=imax; i++) {
...
int buckin_2d(pMesh mesh,pBucket bucket,double *c) {
    ...
    if ( bucket->head[ic] ) {
        ...
        do {
            ...
            jmax = jj+d;
            imin = LS_MAX(0,imin);
            imax = LS_MIN(imax,siz-1);
            jmin = LS_MAX(0,jmin);
            jmax = LS_MIN(siz-1,jmax);
            for (j=jmin; j<=jmax; j++)
                for (i=imin; i<=imax; i++) {
